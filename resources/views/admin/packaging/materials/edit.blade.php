...
<div class="pkg-form-card">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;padding-bottom:16px;border-bottom:1px solid #eef2f7;">
        ...
        <div>
            <div style="font-size:18px;font-weight:800;color:#0f172a;">Edit Material</div>
            <div style="font-size:13px;color:#94a3b8;">{{ $material->name }}</div>
        </div>
    </div>

    <form action="{{ route('packaging.materials.patch') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="tflex tflex-wrap" style="gap:20px;">
            ...
            <div style="flex:0 0 100%;">
                <label class="pkg-label">Material Image</label>
                @if ($material->image)
                    <div style="margin-bottom:10px;">
                        <img id="image-preview" src="{{ asset($material->image) }}"
                             style="max-height:120px;border-radius:12px;border:1px solid #e2e8f0;" alt="">
                    </div>
                @else
                ...
                <small class="ttext-gray-400">Leave blank to keep existing image.</small>
            </div>

            <!-- Name -->
            <div style="flex:1;min-width:220px;">
                <label class="pkg-label">Material Name <span class="ttext-red-500">*</span></label>
                <input type="text" name="name" class="pkg-input browser-default" value="{{ old('name', $material->name) }}" required>
            ...
            <div style="flex:1;min-width:180px;">
                ...
                <input type="text" name="category" class="pkg-input browser-default" value="{{ old('category', $material->category) }}" placeholder="e.g. Box, Bottle, Sachet">
            </div>

            <!-- Unit -->
            <div style="flex:0 0 160px;">
                <label class="pkg-label">Unit</label>
                <input type="text" name="unit" class="pkg-input browser-default" value="{{ old('unit', $material->unit) }}" placeholder="e.g. pcs, rolls">
            </div>

            <!-- Cost per unit -->
            <div style="flex:0 0 200px;">
                <label class="pkg-label">Cost per Unit (₱)</label>
                <input type="number" name="cost_per_unit" class="pkg-input browser-default"
                       value="{{ old('cost_per_unit', $material->cost_per_unit) }}" step="0.0001" min="0">
            </div>
        </div>
