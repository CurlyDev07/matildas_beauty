...
<div class="pkg-form-card">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;padding-bottom:16px;border-bottom:1px solid #eef2f7;">
        ...
        <div>
            <div style="font-size:18px;font-weight:800;color:#0f172a;">New Packaging Purchase</div>
            <div style="font-size:13px;color:#94a3b8;">Record a new purchase and update inventory automatically</div>
        </div>
    </div>
    ...
    <div class="tflex tflex-wrap" style="gap:16px;margin-bottom:24px;">
        ...
        <div style="flex:0 0 180px;">
            ...
            <input type="date" id="purchase_date" class="pkg-input browser-default" value="{{ date('Y-m-d') }}" required>
        </div>
        <div style="flex:0 0 140px;">
            <label class="pkg-label">Tax (₱)</label>
            <input type="number" id="tax" class="pkg-input browser-default" value="0" min="0" step="0.01">
        </div>
        <div style="flex:0 0 140px;">
            <label class="pkg-label">Shipping Fee (₱)</label>
            <input type="number" id="shipping_fee" class="pkg-input browser-default" value="0" min="0" step="0.01">
        </div>
    </div>
    ...
    <div class="pkg-card" style="margin-bottom:24px;">
        <div class="toverflow-x-auto">
            <table class="pkg-table" id="items-table">
                ...
                <tbody id="items-body">
                    <tr id="empty-row">
                        <td colspan="6" class="pkg-center" style="padding:24px;color:#94a3b8;">
                            No items added yet.
                        </td>
                    </tr>
                </tbody>
    ...
    <div style="display:flex;gap:12px;">
        ...
        <a href="{{ route('packaging.purchases') }}" class="pkg-btn pkg-btn-secondary">Cancel</a>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // Button bindings
        $('#add-item-btn').on('click', function () { addItem(); });
        $('#submit-btn').on('click', function () { submitPurchase(); });

        // Recalc when tax/shipping change
        $('#tax, #shipping_fee').on('input', function () { recalcTotal(); });
    });
    ...
    function addItem() {
        ...
        pkgPurchaseItems.push({ material_id: matId, name: matName, unit: unit, quantity: qty, unit_cost: unitCost, total_cost: total });
            ...
            success: function (res) {
                if (res.success) {
                    Swal.fire({ icon: 'success', title: 'Saved!', text: 'Purchase recorded successfully.' })
                        .then(() => location.href = '/admin/packaging/purchases');
                }
            },
            error: function (xhr) {
