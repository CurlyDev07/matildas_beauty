...
<div class="pkg-form-card">
    ...
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:24px;padding-bottom:16px;border-bottom:2px solid #eef2f7;">
        <a href="{{ route('packaging.purchases') }}" class="pkg-action" title="Back" style="flex-shrink:0;width:42px;height:42px;">
            <i class="fas fa-arrow-left" style="font-size:15px;"></i>
        </a>
        <div>
            <div style="font-size:20px;font-weight:800;color:#0f172a;">New Packaging Purchase</div>
            <div style="font-size:14px;color:#94a3b8;margin-top:2px;">Record a new purchase and update inventory automatically</div>
        </div>
    </div>
    ...
    <div style="margin-bottom:28px;">
        ...
        <!-- Selection input panel (shown after clicking a card) -->
                    ...
                    <tbody id="items-body">
                        <tr id="empty-row">
                            <td colspan="7" class="pkg-center" style="padding:32px;color:#94a3b8;font-size:15px;">
                                ...
                                No items added yet. Tap a material above to get started.
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:18px 20px;margin-bottom:28px;">
        ...
        <div class="tflex tflex-wrap" style="gap:16px;">
            ...
            <div style="flex:0 0 190px;">
                ...
                <input type="date" id="purchase_date" class="pkg-input-lg browser-default" value="{{ date('Y-m-d') }}" required>
            </div>
            <div style="flex:0 0 150px;">
                <label class="pkg-label-lg">Tax (₱)</label>
                <input type="number" id="tax" class="pkg-input-lg browser-default" value="0" min="0" step="0.01">
            </div>
            <div style="flex:0 0 150px;">
                <label class="pkg-label-lg">Shipping Fee (₱)</label>
                <input type="number" id="shipping_fee" class="pkg-input-lg browser-default" value="0" min="0" step="0.01">
            </div>
        </div>
    </div>
    ...
    <div style="display:flex;gap:14px;align-items:center;">
        ...
        <a href="{{ route('packaging.purchases') }}" class="pkg-btn pkg-btn-secondary" style="font-size:16px;padding:12px 24px;">Cancel</a>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        $('.modal').modal();
        $('#tax, #shipping_fee').on('input', function () { pkgRecalcTotal(); });
    });
    ...
    function pkgSelectMaterial(id, name, unit, cost, image) {
        ...
        if (pkgSelectedMatId) {
            ...
            if (prev && !pkgIsAdded(pkgSelectedMatId)) prev.classList.remove('selected');
            else if (prev) prev.classList.remove('selected');
        }
    ...
    function pkgIsAdded(matId) {
        return pkgPurchaseItems.some(function(i) { return i && String(i.material_id) === String(matId); });
    }
    ...
    function pkgAddItem() {
        ...
        // If already in list, remove the old entry first (update behavior)
        for (var x = 0; x < pkgPurchaseItems.length; x++) {
            if (pkgPurchaseItems[x] && String(pkgPurchaseItems[x].material_id) === String(pkgSelectedMatId)) {
                var oldRow = document.getElementById('item-row-' + x);
                if (oldRow) oldRow.remove();
                pkgPurchaseItems[x] = null;
                break;
            }
        }

        var total = qty * unitCost;
        var idx   = pkgPurchaseItems.length;

        pkgPurchaseItems.push({
        ...
        var imgHtml = pkgSelectedImage
            ? '<img src="' + pkgSelectedImage + '" style="width:44px;height:44px;border-radius:8px;object-fit:cover;">'
        ...
        row.innerHTML =
            ...
            '<td><div class="pkg-name" style="font-size:15px;">' + pkgSelectedName + '</div></td>' +
            '<td style="font-size:14px;">' + pkgSelectedUnit + '</td>' +
            ...
            '<td class="pkg-center">' +
                '<button type="button" onclick="pkgRemoveItem(' + idx + ', ' + pkgSelectedMatId + ')" ' +
    ...
    function pkgRemoveItem(idx, matId) {
        pkgPurchaseItems[idx] = null;
        var row = document.getElementById('item-row-' + idx);
        if (row) row.remove();

        // Check if the material still has other entries
        var stillAdded = pkgIsAdded(matId);
        if (!stillAdded) {
        ...
        var remaining = pkgPurchaseItems.filter(Boolean).length;
        if (remaining === 0) {
            var emptyRow = document.getElementById('empty-row');
    ...
    function pkgRecalcTotal() {
        var subtotal = pkgPurchaseItems.filter(Boolean).reduce(function(s, i) { return s + i.total_cost; }, 0);
        ...
        var count    = pkgPurchaseItems.filter(Boolean).length;
    ...
    function submitPurchase() {
        var validItems = pkgPurchaseItems.filter(Boolean);
        ...
        $.ajax({
            url:  '{{ route("packaging.purchases.store") }}',
            type: 'POST',
            data: payload,
            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
            success: function (res) {
                if (res.success) {
                    Swal.fire({ icon: 'success', title: 'Saved!', text: 'Purchase recorded successfully.' })
                        .then(function() { location.href = '/admin/packaging/purchases'; });
                }
            },
            error: function (xhr) {
    ...
    $('#submit-btn').on('click', function () { submitPurchase(); });
</script>
@endsection
